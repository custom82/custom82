...
if [[ ${PV} == *9999* ]] ; then
	...
	SRC_URI="
		...
		contrib? (
			...
			https://github.com/opencv/opencv_zoo/archive/refs/tags/4.10.0.tar.gz -> ${PN}_zoo-4.10.0.tar.gz
			contribxfeatures2d? (
...
multilib_src_configure() {
	...
	# ===================================================
		...
		-DCUDNN_INCLUDE_DIR=/opt/cuda/targets/x86_64-linux/include 
		-DCUDNN_LIBRARY=/opt/cuda/targets/x86_64-linux/lib/libcudnn.so 
		-DCUDNN_LIBRARY_DIR=/opt/cuda/targets/x86_64-linux/lib
...
multilib_src_install() {
	...
	if use contribdnn ; then
		insinto /usr/share/opencv4/dnn/models/
		doins "${WORKDIR}"/"${PN}"_zoo-4.10.0/models/face_detection_yunet/face_detection_yunet_2023mar.onnx || die "File not Found"
		doins "${WORKDIR}"/"${PN}"_zoo-4.10.0/models/face_detection_yunet/face_detection_yunet_2023mar_int8.onnx || die "File not Found"
		doins "${WORKDIR}"/"${PN}"_zoo-4.10.0/models/face_recognition_sface/face_recognition_sface_2021dec_int8.onnx || die "File not Found"
		doins "${WORKDIR}"/"${PN}"_zoo-4.10.0/models/face_recognition_sface/face_recognition_sface_2021dec.onnx || die "File not Found"
	fi

	if use wechat-qrcode ; then
		insinto /usr/share/opencv4/wechat_qrcode/
		doins "${WORKDIR}"/"${PN}"_zoo-4.10.0/models/qrcode_wechatqrcode/sr_2021nov.prototxt || die "File not Found"
		doins "${WORKDIR}"/"${PN}"_zoo-4.10.0/models/qrcode_wechatqrcode/sr_2021nov.caffemodel || die "File not Found"
		doins "${WORKDIR}"/"${PN}"_zoo-4.10.0/models/qrcode_wechatqrcode/detect_2021nov.prototxt || die "File not Found"
		doins "${WORKDIR}"/"${PN}"_zoo-4.10.0/models/qrcode_wechatqrcode/detect_2021nov.caffemodel || die "File not Found"
	fi
