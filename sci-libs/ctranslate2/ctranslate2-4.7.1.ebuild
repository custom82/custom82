...
# Dal tuo log: /var/tmp/portage/.../work/ctranslate2-4.7.1
...
src_prepare() {
	...
	mv "${WORKDIR}/spdlog-1.14.1" third_party/spdlog || die
	mv "${WORKDIR}/cpu_features-0.9.0" third_party/cpu_features || die

	# La CLI richiede cxxopts come submodule (third_party/cxxopts)
	if use cli; then
		rm -rf third_party/cxxopts || die
		mv "${WORKDIR}/cxxopts-3.2.0" third_party/cxxopts || die
	fi
